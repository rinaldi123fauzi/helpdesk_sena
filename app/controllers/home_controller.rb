class HomeController < ApplicationController
  def index
    if current_user.roles.any? {|r| r.name == "user"}
      ...
      @status_open = Ticket.where(status: 'created', issued_by: current_user.username).count
      @status_approval = Ticket.where(status: 'approval2', issued_by: current_user.username).count
      ...
      @status_approval = Ticket.where(status: 'approval2').count

class TicketsController < ApplicationController
  ...
  def index
    if current_user.roles.any? {|r| r.name == "user"}
      ...
      @ticket = Ticket.left_outer_joins(:sub_category).where('assigned_by = ? and tickets.status = ?', current_user.username, 'inprogress')
      if @ticket.count == 1
        @tickets = Ticket.left_outer_joins(:sub_category).where('assigned_by = ? and tickets.status = ?', current_user.username, 'inprogress')
      else
